...
export default function Sidebar({ activeTab, setActiveTab, onLogout }) {
  ...
  return (
    <aside className="w-64 bg-[#0b0f19] border-r border-slate-800 flex flex-col h-screen max-h-screen sticky top-0 overflow-hidden shrink-0 select-none">
      {/* Brand Logo Header */}
      ...
      {/* Navigation Menu (Flex Auto Fill & Clean Scroll) */}
        ...
        <div className="flex items-center justify-between p-2.5 rounded-xl bg-slate-900/80 border border-slate-800">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="truncate">
              <p className="text-xs font-semibold text-slate-200 truncate">Akash (Admin)</p>
          ...
          <button 
            onClick={onLogout}
            title="Logout"
            className="text-slate-400 hover:text-red-400 hover:bg-red-500/10 p-2 rounded-lg transition-all duration-200 flex items-center justify-center shrink-0 ml-1"
          >
            <LogOut size={18} />
          </button>
        </div>
